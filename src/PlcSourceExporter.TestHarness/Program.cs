...
using PlcSourceExporter.TiaV20;

try
{
    var options = HarnessOptions.Parse(args);
    if (!string.Equals(options.TiaVersion, "V20", StringComparison.OrdinalIgnoreCase))
    {
        throw new ArgumentException("Only --tia-version V20 is supported by this harness build.");
    }
...
internal sealed class HarnessOptions
{
    ...
    private static void PrintUsage()
    {
        ...
        Console.WriteLine("  PlcSourceExporter.TestHarness.exe --tia-version V20 --project <path-to-.ap20> [--plc-name <name>] [--output UserFiles\\export]");
    }
}
